...
- **좌클릭** — Claude Code 터미널/IDE를 앞으로. 창뿐 아니라 **그 세션의 탭까지** 앞으로
  ...
  JetBrains IDE는 한 단계 위에서 같은 문제가 있어요 — 열어둔 **프로젝트** 전부가 한
  프로세스라서요. 그래서 창 제목의 프로젝트 폴더로 자기 프로젝트 창을 골라 올려요.
- **이름 확인** — 프로젝트마다 펫 하나면 어느 게 어느 건지 모르니, 툴팁(과 우클릭 메뉴
  맨 위)에 알려줘요: `claudlet · my-project — 작업중`. 프로젝트 이름은 그 세션의
  작업 디렉터리에서 가져와요.
